...
function initSite() {
  ...
  // Product inquiry helpers
  window.prepareProductInquiry = function(productName) {
    const field = document.getElementById('category');
    const message = document.getElementById('message');
    if (field) field.value = productName || '';
    if (message && (!message.value || message.dataset.autoGenerated === 'true')) {
      message.value = `I am interested in ${productName}. Please share product details, available packing options, quotation and export information.`;
      message.dataset.autoGenerated = 'true';
    }
  ...
  window.openProductWhatsApp = function(productName) {
    const text = `Hello RS Global Ventures, I am interested in ${productName}. Please share product details, packing options, quotation and export information.`;
    const url = `https://wa.me/919979267148?text=${encodeURIComponent(text)}`;
